...
private fun WidgetContent(snap: TodayWidget) {
    val dark = ColorProvider(Color(0xFF222222))
    val gray = ColorProvider(Color(0xFF777777))
    Column(GlanceModifier.fillMaxSize().background(Color.White).padding(12.dp)) {
        Text(snap.dutyName ?: "오늘 근무 없음",
            style = TextStyle(color = dark, fontWeight = FontWeight.Bold, fontSize = 16.sp))
        if (snap.total > 0) {
            Text("체크 ${snap.done}/${snap.total}", style = TextStyle(color = gray, fontSize = 13.sp))
        }
        snap.nextAlarm?.let {
            Text("⏰ $it", style = TextStyle(color = gray, fontSize = 13.sp))
        }
        if (snap.pendingMemos > 0) {
            Text("📝 메모 ${snap.pendingMemos}", style = TextStyle(color = gray, fontSize = 13.sp))
        }
    }
}
